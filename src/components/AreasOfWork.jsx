...
function AreasOfWork() {
    ...
    return (
        <section id="expertise" className="py-24 px-6 bg-neutral-50 dark:bg-black transition-colors duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {areas.map((area, index) => {
                        ...
                        return (
                            <div
                                key={index}
                                className={`group p-8 rounded-3xl bg-white dark:bg-neutral-900/50 border border-neutral-200 dark:border-neutral-800 transition-all duration-300 hover:bg-white dark:hover:bg-neutral-900 ${area.border} shadow-sm dark:shadow-none hover:shadow-md`}
                            >
                                {/* Header */}
                                <div className="flex items-start justify-between mb-6">
                                    ...
                                    <span className="text-neutral-400 dark:text-neutral-600 text-sm font-mono">
                                        0{index + 1}
                                    </span>
                                </div>
                                ...
                                <p className="text-neutral-600 dark:text-neutral-400 leading-relaxed mb-8 h-20">
                                    {area.description}
                                </p>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {area.tags.map((tag, idx) => (
                                        <span
                                            key={idx}
                                            className="px-3 py-1 text-xs font-medium text-neutral-600 dark:text-neutral-400 bg-neutral-100 dark:bg-neutral-800 rounded-full border border-neutral-200 dark:border-neutral-700/50"
                                        >
                                            {tag}
                                        </span>
                                    ))}
